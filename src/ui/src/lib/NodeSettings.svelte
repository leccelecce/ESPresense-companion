<script lang="ts">
    ...
    import { loadNodeSettings, saveNodeSettings } from '$lib/node';

    export let id: string;

    let loading = true;
    let error: string | null = null;
    let settings: NodeSetting | null = null;
    const toastStore = getToastStore();

    onMount(async () => {
        try {
            settings = await loadNodeSettings(id);
        } catch (e: unknown) {
    ...
    async function save() {
		if (settings && settings.id) {
			try {
				await saveNodeSettings(settings);
			} catch (e) {
				console.log(e);
...
{:else if settings}
    <div class="card m-2 p-4 variant-filled-surface">
        ...
        <div class="flex justify-end mt-8">
            <button class="btn variant-filled-primary" on:click={save}>Update Settings</button>
        </div>
    </div>
{:else}
